  ...
  const associations = [
    {
      ...
      link: "https://udhyog.gujarat.gov.in/",
    },
    {
    ...
    <section className="relative bg-white overflow-hidden section-padding">
      <div className="container mx-auto text-center mb-16">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          GUIITAR COUNCIL collaborates with leading industry partners, government organizations, and academic
          institutions to foster innovation and entrepreneurship.
        </p>
      </div>
        ...
        {associations.map((assoc, index) => (
          ...
          >
            <div
              ...
              style={{ top: 0, left: 0 }}
              onClick={() => handleLogoClick(assoc.link)}
            >
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden shadow-lg border-4 border-white hover:scale-110 hover:shadow-xl transition-all duration-300">
                <img
                  src={assoc.image || "/placeholder.svg"}
                  alt={assoc.name}
                  className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 object-contain rounded-full"
                />
              </div>
              <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                <div className="bg-gray-800 text-white text-xs px-2 py-1 rounded whitespace-nowrap">{assoc.name}</div>
...
// Note: The above code is a React component that displays a section with associations.